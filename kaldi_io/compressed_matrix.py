...
class GlobalHeader(object):
  ...
  def compute(array, compression_method, endian='<'):
    if compression_method == kAutomaticMethod:
      compression_method = kSpeechFeature if array.shape[0] > 8 else kTwoByteAuto
    ...
    if compression_method == kSpeechFeature:
      ...
      raise ValueError('Unknown compression_method: %s' % compression_method)
    ...
    return GlobalHeader(matrix_type, min_value, range_, 
                        array.shape[0], array.shape[1], endian)

  def float_to_uint(self, array):
    dtype = self.endian+'u2' if self.c == 65535. else self.endian+'u1'
...
class PerColHeader(object):
  ...
  def read(fd, global_header):
    ...
    header_array = header_array.reshape(global_header.cols, 4)
    return header_array
  ...
  def compute(array, global_header):
    ...
    if array.shape[0] >= 5:
      ...
      p0   = srows[0]
      p25  = srows[1] if array.shape[0] > 1 else p0 + 1
      p75  = srows[2] if array.shape[0] > 2 else p25 + 1
      p100 = srows[3] if array.shape[0] > 3 else p75 + 1
  ...
  @staticmethod
  def char_to_float(array, p0, p25, p75, p100):
    ma1 = array <= 64
    ma3 = array > 192
