...
class ModelInferencer(object):
    def __init__(self, speaker_model, optimizer, gpu, enroll_frames, test_frames, crop_option, extract_once, **kwargs):
        ...
        self.extract_once = extract_once

        if enroll_frames == test_frames:
            self.equal_len = True
            self.extract_once = True
        else:
    ...
    def evaluateFromList(self, test_trial, test_list, test_path, nDataLoaderThread, \
        ...
        if test_trial == 'O':
            ...
            ## Get a list of file names from subfolder of test path
            files = glob.glob(os.path.join(test_path,'*/*/*/*/*.wav'));
            test_dict = {}
            for x in files:
                test_dict['/'.join(x.strip().split('/')[-3:])] = x

            if self.extract_once:
                ## Define test data loader
                test_dataset = test_dataset_loader_dict(test_dict, **kwargs)
                
            else:  
                enr_files = list(itertools.chain(*[x.strip().split()[-2:-1] for x in lines]))
                enr_list  = list(set(enr_files))
                enr_list.sort()
                

                te_files = list(itertools.chain(*[x.strip().split()[-1:] for x in lines]))
                te_list  = list(set(te_files))
                te_list.sort()

                enr_dict = {}
                for x in enr_list:
                    enr_dict[x] = test_dict[x]

                te_dict = {}
                for x in te_list:
                    te_dict[x] = test_dict[x]

                ## Define enroll and test data loader
                enr_dataset = test_dataset_loader_dict(enr_dict, **kwargs)
                te_dataset  = test_dataset_loader_dict(te_dict, **kwargs)

        if distributed:
            if test_trial == 'O' or self.extract_once:
                sampler = torch.utils.data.distributed.DistributedSampler(test_dataset, shuffle=False)
            else:
                enr_sampler = torch.utils.data.distributed.DistributedSampler(enr_dataset, shuffle=False)
                te_sampler  = torch.utils.data.distributed.DistributedSampler(te_dataset, shuffle=False)
        else:
            if test_trial == 'O' or self.extract_once:
                sampler = None
            else:            
        ...
        if test_trial == 'O' or self.extract_once:
            ...
            enr_loader = torch.utils.data.DataLoader(enr_dataset, batch_size=1, shuffle=False, num_workers=nDataLoaderThread, drop_last=False, sampler=enr_sampler)
            te_loader = torch.utils.data.DataLoader(te_dataset, batch_size=1, shuffle=False, num_workers=nDataLoaderThread, drop_last=False, sampler=te_sampler)
            
            ## Extract features for every utterance from enr_loader
            for idx, data in enumerate(enr_loader):
                inp1 = data[0][0].cuda()
                with torch.no_grad():
                    ref_feat_enr = self.__model__.module.inference(self.crop(inp1,self.enroll_frames,self.crop_option)).detach().cpu()
                feats_enr[data[1][0]] = ref_feat_enr
                ...
                if idx % print_interval == 0 and rank == 0:
                    sys.stdout.write(
                        "\r[Enroll] Reading {:d} of {:d}: {:.2f} Hz, embedding size {:d}".format(idx, enr_loader.__len__(), idx / telapsed, ref_feat_enr.size()[1])
                    )
            print("")
            tstart = time.time()

            ## Extract features for every utterance from te_loader
            for idx, data in enumerate(te_loader):
                inp1 = data[0][0].cuda()
                with torch.no_grad():
                    ref_feat_te = self.__model__.module.inference(self.crop(inp1,self.test_frames,self.crop_option)).detach().cpu()
                feats_te[data[1][0]] = ref_feat_te
                ...
                if idx % print_interval == 0 and rank == 0:
                    sys.stdout.write(
                        "\r[Test] Reading {:d} of {:d}: {:.2f} Hz, embedding size {:d}".format(idx, te_loader.__len__(), idx / telapsed, ref_feat_te.size()[1])
                    )
        ...
        if rank == 0:
            ...
            for idx, line in enumerate(lines):
                ...
                ref_feat = feats_enr[data[1]].cuda()
    ...
    def evaluateFromList_easy_aSn(self, test_trial, test_list, test_path, nDataLoaderThread, \
        ...
        if rank == 0:
            ...
            for idx, line in enumerate(lines):
                ...
                score = 0.5*(score-means_enr[data[1]].cuda()[0])/stds_enr[data[1]].cuda()[0] \
                         + 0.5*(score-means_te[data[2]].cuda()[0])/stds_te[data[2]].cuda()[0]
    ...
    def evaluateFromList_saveSE(self, test_trial, test_list, test_path, nDataLoaderThread, \
        ...
        if rank == 0:
            ...
            for idx, line in enumerate(lines):
                ...
                ref_feat = read_kaldi_binary(utt2arkp_enr[data[1]])
