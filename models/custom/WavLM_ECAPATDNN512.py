...
class WavLM_ECAPA(nn.Module):
    def __init__(self, C = 512, bottleneck = 128, nOut = 192, n_mels = 80, log_input = True, pretrained = None, **kwargs):
        super(WavLM_ECAPA, self).__init__()

        checkpoint = torch.load(pretrained)
        self.wavlm = WavLM(WavLMConfig(checkpoint['cfg']))
    ...
    def wave2emb(self, wave, max_frame=False, aug=False):
        ...
        with torch.no_grad():
            self.wavlm.eval()
            feat = self.wavlm.extract_features(wave)[0]
